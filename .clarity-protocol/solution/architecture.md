...
A Tauri-packaged native app that wraps the web UI and spawns a bundled `clarity-server` sidecar on a local port. Distributable as a standalone macOS, Linux, or Windows application. Uses PyInstaller to bundle the Python runtime; the Tauri shell provides the native window and OS integration.
